...
import { Smartphone, Search, Plus, Activity } from 'lucide-react';
...
const AppNoticePage = () => {
        ...
        { header: "Title", accessor: "title" },
        {
            ...
            render: (row) => (
                <span className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${row.type === 'Alert' ? 'bg-red-100 text-red-700' :
                        row.type === 'Update' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'
                    }`}>
                    {row.type}
                </span>
            )
    ...
    // Mock data for App Notices
    const appNotices = [
        { id: 1, title: "New Feature Release", type: "Update", audience: "All Users", date: "2024-07-20" },
        { id: 2, title: "Scheduled Maintenance", type: "Alert", audience: "All Users", date: "2024-07-18" },
        { id: 3, title: "Performance Improvements", type: "Info", audience: "Android Users", date: "2024-07-15" },
    ];

    const filteredNotices = appNotices.filter(notice =>
        notice.title.toLowerCase().includes(searchText.toLowerCase()) ||
        notice.audience.toLowerCase().includes(searchText.toLowerCase())
    );
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
            ...
            <div className="rounded-[2rem] mt-8 bg-gradient-to-br from-indigo-800 via-purple-700 to-indigo-900 px-6 sm:px-10 lg:px-12 py-8 sm:py-12 flex items-center min-h-[160px] sm:min-h-[200px] shadow-2xl shadow-indigo-200/50 relative overflow-hidden group">
                ...
                <div className="text-left text-white max-w-2xl relative z-10">
                    ...
                    <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold tracking-tight mb-2">App Notices</h1>
                    <p className="text-sm sm:text-base text-purple-100/90 font-medium max-w-lg leading-relaxed">
                        Manage in-app notifications, announcements, and targeted messages for users.
                    </p>
                </div>
            </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-4 text-xs font-medium text-gray-600">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-bold">UPDATE</span>
                            <span>- New features/changes</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full bg-green-500" />
                            <span className="bg-green-100 text-green-700 px-2 py-0.5 rounded font-bold">INFO</span>
            ...
            <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-8 mb-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                    <h2 className="text-xl font-bold text-gray-900 tracking-tight">App Notice History</h2>
                    <button
                        onClick={() => navigate('/app-notice/add')} {/* Assuming an Add App Notice route */}
